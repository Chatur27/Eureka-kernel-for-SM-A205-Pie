...
static int dwc3_gadget_vbus_session(struct usb_gadget *g, int is_active)
{
	...
	if (dwc->gadget_driver && dwc->softconnect) {
		...
		schedule_work(&dwc->pm_qos_work_fsys);
	}
...
static void dwc3_gadget_conndone_interrupt(struct dwc3 *dwc)
{
	...
	case DWC3_DCFG_SUPERSPEED:
		/*
		 * When the core is connected at USB 2.0 speeds, the ram_clk must
		 * attached to the bus clk.
		 * When the core is connected at USB 3.0 speed the ram_clk must
		 * attached to a source that is at least 125 MHz
		 */
		dwc3_update_ram_clk_sel(dwc, speed);

		/* update pm qos lock value for super speed*/
		schedule_work(&dwc->pm_qos_work_fsys);
