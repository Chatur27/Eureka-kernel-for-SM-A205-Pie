...
bool fimc_is_sec_check_eeprom_crc32(char *buf, int position)
{
	...
	if (check_length > 0) {
		...
		pr_warning("Camera[%d]: Skip to check Sensor Cal Data crc32. check_length(%d)\n", position, check_length);
	}
#endif
...
int fimc_is_sec_rom_power_on(struct fimc_is_core *core, int position)
{
	...
	for (i = 0; i < FIMC_IS_SENSOR_COUNT; i++) {
		...
		if ((module) && (position == module->position)) {
			info("%s : Find matched sensor_id (%d)\n", __func__, sensor_id);
			break;
		}
	}
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	if (temp_start_addr >= 0) {
		...
		finfo->sensor_cal_data_size = EEP_XTALK_CAL_DATA_SIZE_FRONT;
#else
		finfo->sensor_cal_data_size = (finfo->sensor_cal_data_start_addr - finfo->sensor_cal_data_end_addr + 1);
#endif
...
int fimc_is_sec_fw_find_rear(struct fimc_is_core *core, int position)
{
	...
	if (*sensor_id == SENSOR_NAME_S5K2P6) {
...
int fimc_is_sec_fw_find_front(struct fimc_is_core *core, int position)
{
	...
	/* default firmware and setfile */
	if (specific->front_sensor_id == SENSOR_NAME_S5K3P8SP) {
		...
		snprintf(finfo->load_setfile_name, sizeof(FIMC_IS_SR846_FRONT_SETF), "%s", FIMC_IS_SR846_FRONT_SETF);
	} else if (specific->front_sensor_id == SENSOR_NAME_S5K5E3) {
		snprintf(finfo->load_setfile_name, sizeof(FIMC_IS_5E3_SETF), "%s", FIMC_IS_5E3_SETF);
		...
		snprintf(finfo->load_setfile_name, sizeof(FIMC_IS_4HA_FRONT_SETF), "%s", FIMC_IS_4HA_FRONT_SETF);
